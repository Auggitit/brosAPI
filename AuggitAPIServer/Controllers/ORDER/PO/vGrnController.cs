...
namespace AuggitAPIServer.Controllers.ORDER.PO
{
    ...
    [ApiController]
    public class vGrnController : ControllerBase
    {
        private readonly AuggitAPIServerContext _context;

        public vGrnController(AuggitAPIServerContext context)
        {
            _context = context;
        }
        ...
        [HttpGet]
        [Route("getGrnLists")]
        public JsonResult GetGrnLists(int? statusId, string? ledgerId, string? fromDate, string? toDate, int globalFilterId, string? search)
        {
            ...
            rtnData = Common.GetResultCount(_context, "vGrn", rtnData);
        ...
        {
            string query = $"SELECT s.pono,s.grndate,s.refno,s.vendorcode,v.\"CompanyDisplayName\",v.\"CompanyMobileNo\",v.\"GSTNo\",v.\"BilingAddress\",sd.product,sd.sku,sd.hsn,sd.qty,sd.rate,(sd.rate * sd.qty) AS total,sd.gstvalue,s.grnno,s.\"cgstTotal\",s.\"sgstTotal\",s.\"igstTotal\",s.\"net\",s.\"expDeliveryDate\",sd.transport,s.contactpersonname,s.phoneno,s.branch,s.fy,s.remarks,s.termsandcondition,c.efieldname,c.efieldvalue FROM public.\"vGrn\" s JOIN \"mLedgers\" v ON Cast(s.vendorcode as int) = v.\"LedgerCode\" JOIN \"vGrnDetails\" sd ON s.grnno = sd.grnno LEFT JOIN \"vGrnCusFields\" c on(c.grnno = s.grnno) WHERE s.\"Id\" = '{id}'";
