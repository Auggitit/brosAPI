...
namespace AuggitAPIServer.Controllers.ORDER.SO
{
    ...
    [ApiController]
    public class vSalesOrderController : ControllerBase
    {
        private readonly AuggitAPIServerContext _context;

        public vSalesOrderController(AuggitAPIServerContext context)
        {
            _context = context;
        }
        ...
        [HttpGet]
        [Route("getSOLists")]
        public JsonResult GetSOLists(int? statusId, string? ledgerId, string? salesRef, string? fromDate, string? toDate, int globalFilterId, string? search)
        {
        ...
        public JsonResult GetSO(string id, bool cusFields)
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                var product = new
                {
                    ...
                    uom = dt.Rows[i][27].ToString(),
                    itemUnderCode = dt.Rows[i][30].ToString(),
                    itemCategoryCode = dt.Rows[i][31].ToString(),
                    groupname = dt.Rows[i][33].ToString(),
                    catname = dt.Rows[i][35].ToString(),
                };
